...
# ===========================================================================
#  SKENARIO 1: sigmoid(raw) + MSELoss  (tanpa MRmD)
#
#  Normalisasi label: sigmoid(shares_raw) langsung
#  Masalah: nilai shares besar → hampir semua saturasi ke ~1.0
#           MSE trivially kecil, model tidak bisa membedakan antar artikel
#
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30'):
    ...
    # [SKENARIO 1] sigmoid(raw) — sigmoid langsung pada nilai shares asli
    # Masalah: nilai shares bisa ribuan-ratusan ribu → saturasi ke ~1.0
    train_labels = (1.0 / (1.0 + np.exp(-train_labels_raw))).astype(np.float32)
    test_labels  = (1.0 / (1.0 + np.exp(-test_labels_raw ))).astype(np.float32)

    n_saturated = int(np.sum(train_labels > 0.9999))
    print(f'[Dataset][Skenario 1] sigmoid(raw):')
    print(f'  Label range train : [{train_labels.min():.6f}, {train_labels.max():.6f}]')
    print(f'  Saturasi (>0.9999): {n_saturated}/{len(train_labels)} '
          f'({100*n_saturated/len(train_labels):.1f}%)')
    ...
    print('[Embedding][Skenario 1] Melatih SupervisedLearnableEmbeddingModel '
