...
# ===========================================================================
#  SKENARIO 2: sigmoid(min-max) + MSELoss  (tanpa MRmD)
#
#  Normalisasi label: min-max ke [0,1] dulu, baru sigmoid
#  Masalah: input sigmoid [0,1] → output [0.5, 0.731] (range 0.231)
#           MSE loss sangat kecil → Reg: 0.0000 di log
#
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30'):
    ...
    # [SKENARIO 2] sigmoid(min-max) — min-max ke [0,1] dulu, baru sigmoid
    # Masalah: input sigmoid hanya [0,1] → output sigmoid [0.5, 0.731]
    # Range efektif hanya 0.231 → MSE trivially kecil → Reg: 0.0000 di log
    label_min   = float(train_labels_raw.min())
    label_max   = float(train_labels_raw.max())
    label_range = label_max - label_min + 1e-8

    train_mm     = (train_labels_raw - label_min) / label_range
    test_mm      = (test_labels_raw  - label_min) / label_range

    train_labels = (1.0 / (1.0 + np.exp(-train_mm))).astype(np.float32)
    test_labels  = (1.0 / (1.0 + np.exp(-test_mm ))).astype(np.float32)

    print(f'[Dataset][Skenario 2] sigmoid(min-max):')
    ...
    print('[Embedding][Skenario 2] Melatih SupervisedLearnableEmbeddingModel '
