...
#
#  Normalisasi label: log1p dulu (tekan skewness ~31 → ~0.99),
#                     lalu min-max ke [0,1] (cocok dengan Sigmoid output)
#  Hasil: range [0,1] penuh, distribusi mendekati normal
#         MSE loss bermakna dan informatif
#
#  Arsitektur: nn.Embedding (cat only) + MLP + LayerNorm
#             → Regressor (Sigmoid output) + MSELoss
...
def load_dataset(dataname, idx=0, mask_type='MCAR', ratio='30'):
    ...
    # [SKENARIO 3] log1p + min-max (REKOMENDASI)
    # log1p menekan skewness ekstrem pada shares (skew ~31 → ~0.99)
    # min-max menyesuaikan range ke [0,1] agar cocok dengan output Sigmoid
    train_log = np.log1p(train_labels_raw)
    test_log  = np.log1p(test_labels_raw)

    label_log_min   = float(train_log.min())
    label_log_max   = float(train_log.max())
    label_log_range = label_log_max - label_log_min + 1e-8

    train_labels = ((train_log - label_log_min) / label_log_range).astype(np.float32)
    test_labels  = ((test_log  - label_log_min) / label_log_range).astype(np.float32)

    train_skew = float(pd.Series(train_labels).skew())
    print(f'[Dataset][Skenario 3] log1p + min-max:')
    print(f'  log_min={label_log_min:.4f}, log_max={label_log_max:.4f}')
    print(f'  Label range train : [{train_labels.min():.4f}, {train_labels.max():.4f}]')
    print(f'  Skewness setelah  : {train_skew:.4f} (sebelum: ~31.0)')
    ...
    print('[Embedding][Skenario 3] Melatih SupervisedLearnableEmbeddingModel '
